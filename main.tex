...
In addition to high-priority schools, Dr. Smith and Tracy have identified several medium-to-low priority schools that formerly housed comprehensive breeding programs.  These schools include: South Carolina State University, North Dakota State University, and the University of Tennessee, and Dr. Tracy may visit them in the second year, pending time commitments and further contacts at these institutions.  
...
By using allele dropping we target individual loci within and across breeding programs, attributing high frequency SNPs with their given selective regime (drought, disease, etc). 
...
Additionally, allele dropping may not have the statistical power to detect a more diffuse polygenic signal of adaptation spread across many alleles in a genome (similar the drawbacks of using traditional QTL mapping and $F_{ST}$ scans) \cite{Rockman:2011ej, Berg:2014bs}. 
...
Briefly, the approach $Q_{x}$ uses SNP hits and their effect sizes across the genome from a GWAS analysis that are associated with phenotypic traits of interest in one environment. 
These hits are then projected to the expected phenotypic (breeding) value based on the genotype of individuals in another environment (where the phenotype has not been observed). 
...
\caption{Graphical timeline of the major parts of our proposal. \kc{will add side figure of table of roles? It might be better pushed down}}
\label{fig:timeline}
\end{SCfigure}
...
Dr. Tracy will, in the second year, visit any of the remaining medium-to-low breeding programs identified as having any more important hard-copy information, and give the scanned information to Dr. Crosby for deposition and curation in the database.

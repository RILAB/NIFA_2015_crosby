...
Ideally, field-testing of these lines would be thorough; containing many replicates of each of the lines grown out in several different environments. However, we are working under space/time constraints and more importantly with limited germplasm obtained from seed repositories.   
Additionally, as we will be working with older germplasm, and it is possible that some lines will fail to germinate, resulting in unequal numbers with which to test our assumptions. 
We think this initial design is a good compromise between being thorough, while still keeping these constraints in mind. 

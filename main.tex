...
Of even greater concern is the possibility that the rate of gain is actually decreasing (Figure \ref{fig:piecewise}).
While some of these differences are likely due to changing management practices, much of the change is indeed due to breeding \citep{Duvick:2001fy}, and current rates of yield gain are lower than historical trends even after correcting for nitrogen fertilizer inputs (data not shown). 
...
Breeding  requires heritable genetic diversity, and efforts to meet future yield goals will require access to sufficient diversity to maintain or increase genetic gain.
Lack of access to genetic diversity may already be slowing breeding gains.
...
As selection progresses within a breeding program, inbred lines with poor combining ability are generally discarded, leading to a decreased effective population size ($N_e$) of the breeding pools.
Response to selection $R$ depends on the additive genetic variance $V_a$ for a trait \citep{kelly2011breeder}:
\begin{align*}
R=\frac{V_a}{V_p}s
\end{align*}
...
\begin{SCfigure}[][ht]
\includegraphics[width=0.5\linewidth]{BSSS.png}
...
Further, much of the pedigree information on older founder inbred maize lines sits in old volumes of Crop Science and Agronomy Journal, experimental station technical bulletins, regional corn breeding meeting records, release sheets, or breeding books, unused and vulnerable to loss. % * <jholland@ncsu.edu> 2015-03-30T19:12:23.592Z:
%
...
%
% ^ <kate.crosby@gmail.com> 2015-03-30T21:15:27.675Z.
...
Undoubtedly, there will be information garnered from historical pedigree information on lines that do not have any genomic data available for them. 
In such cases (where germplasm is available), we intend to use genotype-by-sequencing (GBS) \citep{Elshire:2011ha} as a cost-efficient platform \citep{Glaubitz:2014eu} to obtain genomic data for such lines, with markers aligned to the newest maize reference genome (currently B73 v. 3). 
...
\label{}
Novel population genomic approaches described in \cite{Berg:2014bs} propose the idea of using a technique analogous to $Q_{ST}$- $F_{ST}$ approaches popularized in the 1990s \citep{Latta:1998ek,  McKay:2002wi}, but applied to SNP hits from Genome-Wide-Association-Studies (GWAS). 
...
This lack of signal in GWAS hits may be due to the polygenic nature of many traits, with the genetic basis of each trait explained by many loci of small effect(s), with each locus and effect scattered throughout the genome, and difficult to detect with conventional statistical methods. 
In order to be able to detect these weak signals, \cite{Berg:2014bs} make use of a random set of SNPs 
We propose using hits from GWAS analyses in the present day modern maize lines to project phenotypes from one environment or time period (the present-day) to another environment or time period. 

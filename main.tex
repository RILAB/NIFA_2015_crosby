...
As any breeder will attest, many old lines have fallen out of use precisely because they showed inferior agronomic performance under a particular selective regime (drought, disease resistance, etc) \textcolor{red}{can do a quick summary of what currently has that information}. 
...
As an example, a regular expression ``grep", case-insensitive search of character strings of terms that might typically be associated with selection regimes  (Figure \ref{fig:words}), such as ``resistance'', ``'' .
from our own ancestry data table-joined to data from NPGS reveals that many lines were probably selections of different types of selection programs
...
\begin{figure}[h]
        ...
        \begin{subfigure}[b]{0.55\textwidth}
                \includegraphics[width=\textwidth]{selection_disease.pdf}
                \caption{}
                \label{fig:words}
        \end{subfigure}%
...
\jri{what format is this in? need to mention this has already been computerized and is searchable, etc.  should mention effort gone into standardizing names etc. as well. } 

\par Further work, has led to a total of 1696 unique (1900 duplicate) records overlapping with GBS data, and we have applied the approach of allele-dropping \textcolor{red}{add in basic allele dropping schematic - and then show for some families} to some lines that feature heavily in the pedigree.
...
\jri{summary of goal. explanation of travel -- schools, etc., formatting, standardization, putting in maizegdb. someone needs to be able to read this and imagine what is actually going to take place.}

\jri{new paragraph: genotyping lines. which lines, why, etc. to fill in pedigree.}

\subsubsection*{Expected outcomes and potential pitfalls}
\jri{expected outcomes of database. why is this helpful to BREEDERS and INDUSTRY and COMMUNITY}

\jri{problems/pitfalls of database stuff}

\subsection*{Aim 2: Identify useful diversity}

\jri{brief description of the goal: show methods that can be used to identify beneficial alleles, phenotypes that have been under selection.  these then allow breeders to choose which lines may be of most use to resurrect into their germplasm.}

\subsubsection*{Mendelian segreagation on a pedigree}
\jri{mention pioneer patent on allele dropping (cited above). explain basic idea -- segregation distortion in pedigree == selection. describe data and approach, doing this for different pedigrees selected for different reasons (cite your cool figure).}

\subsubsection*{Predictive phenotyping}
\jri{cite some lit (see above examples) that genotype and pedigree can be used to predict phenotype. here we show that if individual lines predictive phenotype is better than expected given pedigree/genomic background, then it's likely a good choice for breeding! }
...
\jri{explain we don't have feasiability of doing large scale testing, but this is meant as a proof-of-principle to ground-truth our strategy. 

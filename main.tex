...
In contrast, older U.S. inbred lines, though lower-yielding than their contemporaries, harbor novel genetic diversity of potential use for breeding \citep[e.g.][]{chen2012characterization,wisser2011multivariate}, but are already adapted to the temperate U.S.  
...
\begin{figure}[t]
        ...
        \begin{subfigure}[b]{0.5\textwidth}
                ...
                \label{fig:div}
        \end{subfigure}
\caption{a) Counts of a small set of germplasm descriptors from the USDA database for which we currently have genomic and ancestral data. b) Expected segregation of alleles at loci for two traits in different breeding programs.  Selective regime for combining ability across two example breeding programs and loci. Fitness (w) of each allele indicated, with selection coefficient (1-\textit{s}) for the allele being selected against in a given regime. Locus 2, though beneficial under drought conditions, risks loss due to drift in breeding programs focusing only on combining ability. \kc{I think there's also the aspect that the alleles for drought are still preserved, because they are conditionally neutral in the combining program (breeder's selection is relaxed for drought - and yes, they can be lost to drift). These conditionally neutral alleles are preserved against a different genetic background. If we find lots of adaptive alleles in one environment, we might find them in another, but with a different background - introgressions can be planned in that way. I think of this more as local breeder's adaptation - when you cross lines from two or more populations/programs, you'll likely get a bunch of crap hybrids, but you may also get transgressive segregation of all beneficial alleles (subject to some stats - Pioneer can do crazy CTL things to avoid massive amounts of crosses) - this is exactly how heterotic groups work. But here, we're talking about creating lines from way more divergent crosses prior to losing these alleles to drift.}}
\end{figure}
...
While this is never a substitute for growing lines in the relevant environment for comparative purposes (see Aim 3), it doe save considerable time and expense and provides breeders an initial estimate of which germplasm could be most useful.
...
\subsubsection*{Mendelian segregation on a pedigree (Aim 2)}
...
\kc{NEED TO FIX THIS}
If the ``best old'' lines show terrible agronomic performance relative to current ex-PVP inbred varieties, we will know the approach of integrating ``old'' diversity is an inefficient approach to adding diversity to the breeding pool for yield. \kc{Slight comment - in fact, we won't really know until we do crosses - temporal ones, but we don't have the resources for this.}
...
Dr. Tracy and Dr. James Holland's masters student will be responsible for traveling to and scanning hard-copy records into digital format.  We estimate that this should take no longer than a few weeks at each institution for Dr. Holland's student and Dr. Tracy.

The scanned digital records will then be translated into a usable data format for further analysis by Dr. Crosby.  At worst, this will involve Holland's student and the postdoc (Crosby), physically reading and translating each of the scanned papers, and this could take several months to the entire first year depending on volume of records. Dr. Crosby will ensure that the database is up an running by the end of the first year, and act as database manager throughout the proposal to the end of the third year.
...
At the end of the first year/start of the second year, any inbred line that is disproportionately present or absent in the pedigree with available germplasm, but that has no to little genetic information on it will be genotyped using the GBS platform - Dr. Crosby will be responsible for this aspect of the project, and Holland's student will receive training in GBS analysis from Dr. Crosby. 

Dr. Holland's student will then begin to apply the technique of allele-dropping to various pedigrees, investigating which alleles undergo large directional shifts (in terms of allele frequency changes, generation-to-generation). He/she will also be responsible for obtaining available germplasm with the help of Dr. Flint-Garcia from various repositories for those lines that have no genetic information on them at present. 

Dr. Randall Wisser's student will then start and visit the two land grant schools in the east (Virginia Tech and the University of Pennsylvania) to obtain the remaining records, and submit this phenotypic and pedigree information to the up-and-running database in the first 2-3 months. 

Dr. Wisser's student will grow out the lines identified by Holland's student from the allele-dropping method and phenotype these lines for production related traits (flowering time, kernel size, kernel weight, etc), while at the same time (over a period of 2-3 months) learn the population genomics approach presented in \citep{Berg:2014bs}. 
\kc{Worth it to add in part about SFG phenotyping system? This is also something Sherry could consult on}
In the final year, Dr. Wisser's student will apply the $Q_{x}$ method, and choose the lines that show the greatest over-dispersion for production and production related traits of interest to grow out at Delaware.
...
Our proposal helps train two students in bioinformatics, population and quantitative genetics, plant-breeding, experimental design, statistics, and database management. All these applied skills that the students develop will help them in their future careers, be they academic or industry careers. They will both meet and work with some of the premier experts in maize breeding (Drs. Tracy \& Smith). Both students will be expected and encouraged to attend the maize genetics conference, to present their results, and network with working professionals from academia, government, and industry.

Both students will work alongside the post-doc, Dr. Crosby as well as their respective adivsors to plan out details, troubleshoot, and advance project goals. Dr. Crosby, in turn, will gain valuable experience helping to mentor students along with their main advisors Drs. Wisser and Holland. 

\kc{Jeff, say something about yourself?}

\subsubsection*{Benefits to maize breeding and genomics}
\subsubsection*{Novel diversity from old lines}
With combined and interactive selective pressures of climate change, disease, drought, and pests many crops must quickly adjust to these new challenges. Our proposal seeks out novel diversity from old breeding programs; breeding programs where lines were generally exposed to one selective stress. It then identifies and tags beneficial alleles under a given stress, and allows breeders easy access to this information in other lines, so that they can make their own choices on crosses.

\subsubsection*{Public Pedigree database}
Private companies such as Monsanto and Dupont Pioneer have their own pedigree databases and information that they can draw from in order to breed new hybrid commercial varieties. Breeders with less financial means (both in the United States and in countries abroad) do not have access to this information, nor do public researchers. 

Our proposal aims to make this information accessible, and by hosting on maize GDB, will allow the information to be updated as public breeders themselves try out lines, and make their own observations, well beyond the timeline of this three-year project.
Additionally, Dupont Pioneer (see letter of support) recognizes that much the historical information on maize inbred lines is disjointed, incomplete, scattered across many institutions, and vulnerable to loss. So, this information is a valuable contribution to private industrial databases too.

\kc{I could probably do the below, but it might be overkill}
\subsubsection*{Open-source R package for database}
The post-doc, Dr. Crosby while being responsible for database management and development, will also construct an open-source R-package. This package will allow users to access real-time pedigree information from the database with a suite of functions via an application program interface (API). The R-statistical programming language is open-source and one of the fastest growing languages in the United States and countries abroad for data analysis. 

\subsubsection*{Application to other crop systems}
Few other crops have attempted to make use of old diversity as novel diversity, in a comprehensive project, such as the one we propose here. 
Yet, useful diversity does exist in old lines \citep{gamuyao2012protein}. 

We think a full examination of this old diversity in one crop system (maize) may act as a stimulus for other researchers in other crops to act quickly to preserve hard-copy records, and germplasm associated with these records too. Our proposal, is in essence, a litmus test of how old diversity could be useful in propagating new modern lines in many crops. 

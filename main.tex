...
This method has already been employed by industry to identify useful alleles in their own pedigrees \citep{sebastian1995method}. \textcolor{violet} {As sample sizes may be small within a given pedigree, we recognize that distorted segregation of alleles from parents to offspring may be due to drift alone. To account for the role of genetic drift, Dr. Crosby will perform simulations within and across pedigrees, with slight modifications to the code here: (\url{https://github.com/kate-crosby/simulation_epistasis}).} 
...
First, we will choose a single trait (for simplicity likely a disease resistance) for which we selection was undertaken in a large number of programs in our database.
